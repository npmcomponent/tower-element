...
// commented out by npm-component: var attr = require('tower-attr-directive');
...
exports.render = function(el, parent, prototype, parentScope, copiedAttrs){
  var scope = this.content.init({}, parentScope);
  el.__scope__ = scope;
  
  el.setAttribute = function(name, val){
    setAttribute.call(el, name, val);
    if (scope.constructor.attrs[name]) {
      scope.set(name, val); 
    }
    return val;
  };
  ...
  if (this.el) {
    ...
    for (var i = 0, n = attrs.length; i < n; i++) {
      if (el.hasAttribute(attrs[i].name.toLowerCase()))
        scope.set(attrs[i].name, el.getAttribute(attrs[i].name.toLowerCase()));
    }

    // scope -> el: set the initial values.
    // XXX: doesn't need to happen
    var data = scope.data;
    for (var key in data) {
      if (!el.hasAttribute(key)) el.setAttribute(key, data[key]);
    }

    for (var name in copiedAttrs) {
      el.setAttribute(name, copiedAttrs[name]);
    }
    // XXX: replace
    var clonedEl = this.el.cloneNode(true);
    clonedEl.__scope__ = scope;
    //el.appendChild(clonedEl);
    // XXX: only want this to happen in some cases
    // XXX: should happen on the `el` itself, just should skip the directives
    // already processed
    var templateEl = this.fn(el.__scope__, clonedEl);

    el.appendChild(templateEl);

    for (var i = 0, n = el.childNodes.length; i < n; i++) {
      el.childNodes[i].__scope__ = scope;
    }
  }
